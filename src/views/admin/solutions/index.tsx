...
export default function SolutionsAdminView() {
  ...
  const [{ data, error }, refetch] = useAxios({ url: '/api/solutions', params: { q: query, page: page + 1, pageSize: rowsPerPage, locale, published: publishedOnly ? 'true' : undefined } });
      ...
      <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} alignItems={{ md: 'center' }} justifyContent="space-between" mb={2}>
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5}>
          ...
          <Stack direction="row" spacing={1} alignItems="center">
            <Chip size="small" label={t('published_only') as any || 'Published only'} onClick={() => setPublishedOnly((v) => !v)} color={publishedOnly ? 'success' : 'default'} variant={publishedOnly ? 'filled' : 'outlined'} clickable />
          </Stack>
            ...
            {items.length === 0 && (
              <TableRow>
                <TableCell colSpan={4} align="center">
                  <Typography variant="body2" color="text.secondary">{t('no_items') as any}</Typography>
                </TableCell>
              </TableRow>
            )}
            {items.map((row) => (
