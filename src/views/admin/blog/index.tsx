...
export default function BlogAdminView() {
  ...
  const [{ data, error }, refetch] = useAxios({ url: '/api/blog', params: { q: query, page: page + 1, pageSize: rowsPerPage, locale } });
  const [, createReq] = useAxios({ url: '/api/blog', method: 'POST' }, { manual: true });
  ...
  const onSubmit = async (values: any) => {
    try {
      if ((initial as any)?.id) await updateReq({ url: `/api/blog/${(initial as any).id}`, data: values });
  ...
  const onDelete = async (row: Row) => {
    try {
      await deleteReq({ url: `/api/blog/${row.id}` });
      await refetch();
    } catch (err: any) {
      if (axios.isCancel?.(err) || err?.code === 'ERR_CANCELED' || err?.message === 'canceled') return;
      throw err;
  ...
  return (
    <Container sx={{ py: 3 }}>
      <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} alignItems={{ md: 'center' }} justifyContent="space-between" mb={2}>
        <Box>
          <Typography variant="h5" fontWeight={700}>{t('blog')}</Typography>
          <Typography variant="body2" color="text.secondary">{t('blog_admin_sub')}</Typography>
        </Box>
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5}>
          <Button variant="contained" color="secondary" onClick={onAdd}>{t('add_post')}</Button>
        </Stack>
      </Stack>
      ...
      <PostDialog open={open} onClose={() => setOpen(false)} initial={initial} onSubmit={onSubmit} />
      <ConfirmDialog open={confirm.open} onClose={() => setConfirm({ open: false })} onConfirm={async () => { if (confirm.row) await onDelete(confirm.row); }} title={t('delete_post_title')} message={t('delete_post_message')} />
    </Container>
  );
}
