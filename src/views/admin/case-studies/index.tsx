...
export default function CaseStudiesAdminView() {
  ...
  const [{ data, error }, refetch] = useAxios({ url: '/api/case-studies', params: { q: query, page: page + 1, pageSize: rowsPerPage, locale } });
  ...
  const onDelete = async (row: Row) => {
    try {
      await deleteReq({ url: `/api/case-studies/${row.id}` });
      await refetch();
    } catch (err: any) {
      if (axios.isCancel?.(err) || err?.code === 'ERR_CANCELED' || err?.message === 'canceled') return;
      throw err;
