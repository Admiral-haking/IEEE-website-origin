...
export default function TeamMembersAdminView() {
  ...
  const [{ data, error }, refetch] = useAxios({ url: '/api/team', params: { q: query, page: page + 1, pageSize: rowsPerPage, locale } });
  ...
  const onDelete = async (row: any) => {
    try {
      await deleteReq({ url: `/api/team/${row.id}` });
      await refetch();
    } catch (err: any) {
      if (axios.isCancel?.(err) || err?.code === 'ERR_CANCELED' || err?.message === 'canceled') return;
      throw err;
  ...
  return (
    <Container sx={{ py: 3 }}>
      ...
      <AdminAccessAlert error={error} t={t as any} sx={{ mb: 2 }} />
      <TableContainer>
        <Table size="medium">
          <TableHead>
            <TableRow>
