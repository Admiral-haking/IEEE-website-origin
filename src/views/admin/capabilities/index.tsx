...
type Row = { id: string; title: string; slug?: string; area: 'software'|'hardware'|'networking'|'devops'; description?: string; contentHtml?: string };
...
export default function CapabilitiesAdminView() {
  ...
  const [{ data, error }, refetch] = useAxios({ url: '/api/capabilities', params: { q: query, page: page + 1, pageSize: rowsPerPage, locale } });
  const [, createReq] = useAxios({ url: '/api/capabilities', method: 'POST' }, { manual: true });
  ...
  const onSubmit = async (values: any) => {
    try {
      if ((initial as any)?.id) await updateReq({ url: `/api/capabilities/${(initial as any).id}`, data: values });
  ...
  const onDelete = async (row: Row) => {
    try {
      await deleteReq({ url: `/api/capabilities/${row.id}` });
      await refetch();
    } catch (err: any) {
      if (axios.isCancel?.(err) || err?.code === 'ERR_CANCELED' || err?.message === 'canceled') return;
      throw err;
  ...
  return (
    <Container sx={{ py: 3 }}>
      <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} alignItems={{ md: 'center' }} justifyContent="space-between" mb={2}>
        <Box>
          <Typography variant="h5" fontWeight={700}>{t('capabilities')}</Typography>
          <Typography variant="body2" color="text.secondary">{t('capabilities_admin_sub')}</Typography>
        </Box>
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5}>
          <TextField size="small" placeholder={t('search_capabilities') as string} value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') refetch(); }} />
          <Button variant="contained" color="secondary" onClick={onAdd}>{t('add_capability')}</Button>
        </Stack>
      </Stack>
        ...
        <Table size="medium">
          <TableHead>
            <TableRow>
              ...
              <TableCell>{t('area_label')}</TableCell>
          ...
          <TableBody>
            {items.map((row) => (
              <TableRow key={row.id} hover>
                ...
                <TableCell>{t(row.area)}</TableCell>
      ...
      <CapabilityDialog open={open} onClose={() => setOpen(false)} initial={initial} onSubmit={onSubmit} />
      <ConfirmDialog open={confirm.open} onClose={() => setConfirm({ open: false })} onConfirm={async () => { if (confirm.row) await onDelete(confirm.row); }} title={t('delete_capability_title')} message={t('delete_capability_message')} />
    </Container>
  );
}
