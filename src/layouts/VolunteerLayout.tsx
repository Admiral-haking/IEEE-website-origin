...
export default function VolunteerLayout({ children }: { children: React.ReactNode }) {
  ...
  const nav = (
    <Box role="presentation" sx={{ width: { xs: drawerWidth, md: '100%' }, display: 'flex', flexDirection: 'column', height: '100%' }}>
      <Toolbar sx={{ justifyContent: 'space-between' }}>
        <Typography variant="subtitle1" fontWeight={600} suppressHydrationWarning>{t('user_role_volunteer')}</Typography>
      ...
      <Divider />
      <List>
        <ListItemButton component={NextLink} href={`/${locale}/volunteer`} sx={{ borderRadius: 2, mx: 1, my: 0.5 }}>
  ...
  return (
    <Box sx={{ display: 'flex' }}>
      <Box component="nav" sx={{ width: { md: drawerWidth }, flexShrink: { md: 0 } }} aria-label="volunteer sidebar">
      ...
      <Box component="main" sx={{ flexGrow: 1 }}>
        <Toolbar sx={{ display: { md: 'none' }, justifyContent: 'space-between' }}>
          ...
          <Stack direction="row" spacing={1} alignItems="center">
            <Typography variant="subtitle2" suppressHydrationWarning>{t('user_role_volunteer')}</Typography>
            <Link component={NextLink} href={`/${locale}`} underline="hover" color="inherit" sx={{ fontSize: 12 }} suppressHydrationWarning>{t('home') || 'Home'}</Link>
          </Stack>
        </Toolbar>
        <Box sx={{ p: 2 }}>
