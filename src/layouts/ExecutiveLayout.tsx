...
export default function ExecutiveLayout({ children }: { children: React.ReactNode }) {
  ...
  const nav = (
    <Box role="presentation" sx={{ width: { xs: drawerWidth, md: '100%' }, display: 'flex', flexDirection: 'column', height: '100%' }}>
      <Toolbar sx={{ justifyContent: 'space-between' }}>
        <Typography variant="subtitle1" fontWeight={600} suppressHydrationWarning>{t('user_role_executive')}</Typography>
      ...
      <Divider />
      <List>
        <ListItemButton component={NextLink} href={`/${locale}/executive`} sx={{ borderRadius: 2, mx: 1, my: 0.5 }}>
        ...
        {canMembership && (
          <ListItemButton component={NextLink} href={`/${locale}/executive/membership`} sx={{ borderRadius: 2, mx: 1, my: 0.5 }}>
            <ListItemIcon><HowToRegOutlinedIcon /></ListItemIcon>
            <ListItemText primary={t('membership')} secondary={t('membership_review_desc')} primaryTypographyProps={{ suppressHydrationWarning: true }} secondaryTypographyProps={{ suppressHydrationWarning: true }} />
          </ListItemButton>
        )}
        {canContact && (
          <ListItemButton component={NextLink} href={`/${locale}/executive/contact-messages`} sx={{ borderRadius: 2, mx: 1, my: 0.5 }}>
            <ListItemIcon><MailOutlineIcon /></ListItemIcon>
            <ListItemText primary={t('messages')} secondary={t('nav_messages_sub')} primaryTypographyProps={{ suppressHydrationWarning: true }} secondaryTypographyProps={{ suppressHydrationWarning: true }} />
          </ListItemButton>
        )}
      </List>
    </Box>
  ...
  return (
    <Box sx={{ display: 'flex' }}>
      <Box component="nav" sx={{ width: { md: drawerWidth }, flexShrink: { md: 0 } }} aria-label="executive sidebar">
      ...
      <Box component="main" sx={{ flexGrow: 1 }}>
        <Toolbar sx={{ display: { md: 'none' }, justifyContent: 'space-between' }}>
          ...
          <Stack direction="row" spacing={1} alignItems="center">
            <Typography variant="subtitle2" suppressHydrationWarning>{t('user_role_executive')}</Typography>
            <Link component={NextLink} href={`/${locale}`} underline="hover" color="inherit" sx={{ fontSize: 12 }} suppressHydrationWarning>{t('home') || 'Home'}</Link>
          </Stack>
        </Toolbar>
        <Box sx={{ p: 2 }}>
